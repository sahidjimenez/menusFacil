...
const OrderPizza = (props) => {
    const { dataPizza } = useProductContext();
    ...
    const categoriasUnicas = [...new Set(dataPizza.map(item => item.attributes.categoria))];
    ...
    return (
        ...
        <div className=" container m-auto max-w-[1200px] " >
            ...
            <div className='text-xl text-center p-5 pt-0 pb-1 '>Nombre Restaurante</div>
            <Bar data={dataPizza} />
            ...
            {dataPizza.map((item, index) => {
                ...
                return (
                    <div className='grid py-2 m-auto max-w-[600px]' key={index}>
                        ...
                        <ModalPizza data={item} indice={index} mostrarCategoria={mostrarCategoria} />
                    </div>
                )
            })}
            <ButtonTopView />
