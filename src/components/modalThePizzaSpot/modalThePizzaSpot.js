...
const ModalThePizzaSpot = (props) => {
    ...
    var handleChange = (selectedOption) => {
        ...
        const sumarLabels = (selectedOption) => {
            const labels = selectedOption.label;
        ...
        const sumarPrecios = (selectedOption) => {
            const totalPrecio = selectedOption.precio;
    ...
    const onSubmit = (data) => {
        if (data.tamano === "vaso") {
            ...
            ReactGA.event({
                category: "Ecommerce", // Puedes personalizar esta categoría.
                action: "add_to_cart",
                label: name, // Nombre del producto para identificación fácil.
                value: priceNew, // Precio del producto.
                items: [{ // Detalles del producto según la estructura de GA4.
                    item_name: name,
                    price: priceNew,
                    quantity: 1 // Asumimos una cantidad de 1 para el ejemplo.
                }]
            });
        }
    }
    ...
    const abrirModal = () => {
        ...
        ReactGA.event({
            category: 'Producto',
            action: 'Ver producto',
            label: `${'The Pizza Spot'} - ${name}`,
            nonInteraction: false
        });
        ReactGA.event({
            category: "Ver producto",
            action: "se hizo una vista de " + name,
            label: "visualizacion",
            value: priceNew,
             nonInteraction: false
        })
    ...
    const button = (
        <div>
            {mostrarCategoria && (
                <Element className='text-center my-4 ' name={data.attributes.categoria}>
                    <hr className='w-[100%] h-[4px] mx-auto  bg-yellow-500 border-0 rounded md:my-10'></hr>
                    <div className='flex h-[35px] bg-[#FDBD37] w-[50%] items-center justify-center'>

                        <span className='text-lg font-bold capitalize  text-center'  >{data.attributes.categoria}</span>
                    </div>
                </Element>
            )}
            <button
            ...
            >
                {/* aqui */}
                <div className={classes.fondoTheSpot}>
                    <div className='grid content-baseline gap'>
                        <p className='capitalize text-left font-bold text-xl'>
                            {name}
                        </p>
                        <div className='flex font-medium gap-2 text-md font-bold'>
                            ...
                            {opciones === null
                                ...
                                : <div className="grid grid-cols-3 grid-rows-auto p-6  gap-3 ">
                                    ...
                                    {(opciones && opciones.ingredientes.length > 0 && !opciones.baguette && !opciones.wraps) ?
                                        <div className='col-span-3 grid grid-cols-3 gap-3 row-span-1'>
                                            <p className='col-span-3'>Pizza {opciones.cantidadIngredientes} Ingredientes</p>
                                            {opciones && opciones.ingredientes.map((element, index) => {
                                                return <div className='flex items-center gap-2' key={index}>
                                    ...
                                    {opciones && opciones.extras.length > 0
                                        ...
                                        ? <div className="container col-span-3">
                                            <div className="mt-5 m-auto w-100">
                                                <p className='col-span-3 my-2'>Agregar extra</p>
                                                <Select
...
export default ModalThePizzaSpot